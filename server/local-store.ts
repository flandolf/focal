...
/** SQLite-backed store for the single-user desktop sidecar. */
export class SqliteKeyValueStore<T> implements KeyValueStore<T> {
  constructor(
    ...
    database.run(`
      CREATE TABLE IF NOT EXISTS focal_chatgpt_kv (
  ...
  get(key: string): T | undefined {
    const row = this.database
      .query("SELECT value, expires_at AS expiresAt FROM focal_chatgpt_kv WHERE namespace = ? AND key = ?")
  ...
  set(key: string, value: T, options: { ttlMs?: number } = {}): void {
    ...
    this.database
      .query(`
        INSERT INTO focal_chatgpt_kv (namespace, key, value, expires_at)
        VALUES (?, ?, ?, ?)
  ...
  delete(key: string): void {
    this.database
      .query("DELETE FROM focal_chatgpt_kv WHERE namespace = ? AND key = ?")
