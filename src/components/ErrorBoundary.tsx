...
import { AlertTriangle, RotateCcw, RefreshCw, Copy, Check, Terminal, Bug, ChevronRight } from "lucide-react"
...
export class ErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  state: ErrorBoundaryState = { hasError: false, error: null, errorInfo: null, resetKey: 0 }
  ...
  return (
    <div className="focal-shell relative flex h-screen w-full flex-col overflow-hidden text-foreground">
      ...
      <div
        ...
        className="app-titlebar-drag-region absolute inset-x-0 top-0 z-20 h-[var(--app-titlebar-inset)]"
        aria-hidden
      />

      {/* Background layers */}
      <div className="hairline-grid pointer-events-none absolute inset-0 opacity-80" aria-hidden />
      <div className="ambient-shell pointer-events-none absolute inset-0 z-0 opacity-60" aria-hidden />

      {/* Main content */}
      <div className="relative z-10 flex flex-1 items-center justify-center px-4 py-8 pt-[var(--app-titlebar-inset)]">
        <motion.div
          role="alert"
          aria-live="assertive"
        ...
        >
          ...
          >
            ...
            <p className="mt-2 max-w-sm text-sm leading-relaxed text-muted-foreground" style={{ textWrap: "balance" }}>
              Focal hit an unexpected error and the current view couldn't recover. Your data is safe on disk — try again, or reload the app if the problem persists.
            </p>
          </motion.div>

          {/* Dev badge in dev mode */}
          {showDevDetails && (
            <motion.div variants={staggerItem} className="mt-4 flex justify-center px-6">
            ...
            <AnimatePresence initial={false}>
              {showDetails && (
                <motion.div
                  ...
                  transition={reduceMotion ? REDUCED_TRANSITION : { duration: 0.25, ease: "easeOut" }}
                  className="overflow-hidden"
                >
                  <div className="mt-2 space-y-2 rounded-lg border border-border/60 bg-muted/30 p-3">
                    {/* Error name + message */}
                    <div className="flex items-start justify-between gap-2">
                      <p className="break-words font-mono text-[0.625rem] leading-relaxed text-muted-foreground">
                    ...
                    {showDevDetails && error?.stack && (
                      <div className="relative">
                        <pre className="max-h-48 overflow-auto rounded-md bg-background/60 p-2.5 whitespace-pre-wrap break-words font-mono text-[0.5625rem] leading-relaxed text-muted-foreground/80">
                          {error.stack}
                        </pre>
                      </div>
                    )}
                    ...
                    {showDevDetails && errorInfo?.componentStack && (
                      <div className="relative">
                        ...
                        <pre className="max-h-32 overflow-auto rounded-md bg-background/60 p-2.5 whitespace-pre-wrap break-words font-mono text-[0.5625rem] leading-relaxed text-muted-foreground/80">
                          {errorInfo.componentStack}
                        </pre>
                      </div>
                    )}
                    ...
                    {!showDevDetails && (
                      <p className="text-[0.5625rem] leading-relaxed text-muted-foreground/50">
                        Open the browser console (F12) to view the full stack trace.
                      </p>
                    )}
                  </div>
                </motion.div>
